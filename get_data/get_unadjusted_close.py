"""
Fetch non-adjusted close data from Alpha Vantage API
Uses TIME_SERIES_DAILY endpoint to get all available data for each ticker
...
API_FUNCTION = 'TIME_SERIES_DAILY'  # Non-adjusted daily data
...
def fetch_ticker_data(ticker: str, api_key: str) -> Optional[Dict]:
    """
    Fetch TIME_SERIES_DAILY data for a single ticker
...
def parse_ticker_data(api_response: Dict, ticker: str) -> pd.DataFrame:
    ...
    Note: TIME_SERIES_DAILY does not include adjusted_close, dividend, or split_coefficient
    """
...
def save_ticker_data(df: pd.DataFrame, ticker: str, data_type: str):
    ...
    output_dir = os.path.join(UNADJUSTED_DATA_DIR, data_type)
...
def fetch_all_tickers(data_type: str, api_key: str, tickers: Optional[List[str]] = None):
    """
    Fetch non-adjusted close data for all tickers
    Args:
    ...
    """
    print("="*60)
    print(f"Alpha Vantage Non-Adjusted Close Data Fetcher - {data_type.upper()}")
    print("="*60)
    print()
    ...
    os.makedirs(os.path.join(UNADJUSTED_DATA_DIR, data_type), exist_ok=True)
    ...
    for i, ticker in enumerate(tickers, 1):
        ...
        existing_file = os.path.join(UNADJUSTED_DATA_DIR, data_type, f'{ticker}.csv')
    ...
    if successful:
        print(f"✓ Data saved to: {os.path.join(UNADJUSTED_DATA_DIR, data_type)}")
...
def main():
    """Main entry point"""
    if len(sys.argv) < 2:
        print("Usage: python unadjusted_close.py [spac|despac] [ticker1 ticker2 ...]")
        print()
        print("Options:")
        ...
        print("  Example: python unadjusted_close.py spac AAPL TSLA")
        print()
        print("Environment:")
    ...
    if data_type not in ['spac', 'despac']:
        ...
        print("Usage: python unadjusted_close.py [spac|despac]")
        sys.exit(1)
